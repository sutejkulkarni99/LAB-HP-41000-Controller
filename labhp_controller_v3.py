...
- Plot channel toggles and save image (current screen or full data)
- Configurable plot appearance (background, grid, labels, legend, title)
...
- Per-setpoint reset buttons (voltage/current/power/OVP)
- Global "Reset All Setpoints" button
- Visual limit-active indicators (current/power)
...
# =============================================================================
# LOGGING THREAD (temp file, rename on stop, pause/resume)
# =============================================================================
class LoggingThread(QThread):
    log_line = pyqtSignal(str)
    error = pyqtSignal(str)

    def __init__(self, controller: LABHPController, interval: float, temp_path: str,
...
# =============================================================================
# MAIN WINDOW (enhanced with all features)
# =============================================================================
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Load settings (plot appearance, etc.)
        self._load_settings()
    ...
    def _cleanup_temp_files(self):
        """Delete any leftover lab_hp_log_temp_*.csv files from previous runs."""
        temp_pattern = "lab_hp_log_temp_*.csv"
        for f in Path.cwd().glob(temp_pattern):
            try:
                f.unlink()
    ...
    def _build_control_tab(self, parent):
        ...
        # Global reset
        self.btn_reset_all = QPushButton("Reset All Setpoints")
    ...
    def _build_logger_tab(self, parent):
        ...
        cfg_layout.addWidget(QLabel("Log File Base Name:"))
        ...
        # Plot area with settings
        splitter = QSplitter(Qt.Orientation.Vertical)

        # Plot widget with controls
        plot_container = QWidget()
        plot_layout = QVBoxLayout(plot_container)
        plot_layout.setContentsMargins(0, 0, 0, 0)

        # Plot settings group
        plot_settings_box = QGroupBox("Plot Appearance")
        plot_settings_box.setObjectName("plot_settings_box")
        ps_layout = QFormLayout(plot_settings_box)

        self.combo_bg_color = QComboBox()
        self.combo_bg_color.addItems(["Dark", "White"])
        self.combo_bg_color.setCurrentText("Dark")
        self.combo_bg_color.currentTextChanged.connect(self._update_plot_appearance)
        ps_layout.addRow("Background:", self.combo_bg_color)

        self.chk_grid = QCheckBox("Show grid")
        self.chk_grid.setChecked(False)
        self.chk_grid.stateChanged.connect(self._update_plot_appearance)
        ps_layout.addRow("Grid:", self.chk_grid)

        self.chk_legend = QCheckBox("Show legend")
        self.chk_legend.setChecked(True)
        self.chk_legend.stateChanged.connect(self._update_plot_appearance)
        ps_layout.addRow("Legend:", self.chk_legend)

        self.edit_axis_x = QLineEdit("Time (s)")
        self.edit_axis_x.textChanged.connect(self._update_plot_appearance)
        ps_layout.addRow("X Label:", self.edit_axis_x)

        self.edit_axis_y = QLineEdit("Value")
        self.edit_axis_y.textChanged.connect(self._update_plot_appearance)
        ps_layout.addRow("Y Label:", self.edit_axis_y)

        self.edit_title = QLineEdit("")
        self.edit_title.setPlaceholderText("Plot title (optional)")
        self.edit_title.textChanged.connect(self._update_plot_appearance)
        ps_layout.addRow("Title:", self.edit_title)

        plot_layout.addWidget(plot_settings_box)
        ...
        # Toggle lines (reuse existing checkboxes)
        ...
        plot_layout.addWidget(self.btn_save_plot)
        ...
        # Apply initial plot appearance (will be called again after settings load)
    ...
    def _load_settings(self):
        ...
        # Logging settings
        log_base = self.settings.value("log_base_name", "")
        if log_base:
            self.log_path_edit.setText(log_base)
        ...
        # Plot appearance
        bg_color = self.settings.value("plot_bg", "Dark")
        self.combo_bg_color.setCurrentText(bg_color)
        self.chk_grid.setChecked(self.settings.value("plot_grid", False, type=bool))
        self.chk_legend.setChecked(self.settings.value("plot_legend", True, type=bool))
        self.edit_axis_x.setText(self.settings.value("plot_xlabel", "Time (s)"))
        self.edit_axis_y.setText(self.settings.value("plot_ylabel", "Value"))
        self.edit_title.setText(self.settings.value("plot_title", ""))

        # Apply appearance after all values are set
        self._update_plot_appearance()
    ...
    def _save_settings(self):
        ...
        self.settings.setValue("plot_bg", self.combo_bg_color.currentText())
        self.settings.setValue("plot_grid", self.chk_grid.isChecked())
        self.settings.setValue("plot_legend", self.chk_legend.isChecked())
        self.settings.setValue("plot_xlabel", self.edit_axis_x.text())
        self.settings.setValue("plot_ylabel", self.edit_axis_y.text())
        self.settings.setValue("plot_title", self.edit_title.text())
    ...
    def _reset_ovp(self):
        default_ovp = 1100.0
        self.spin_ovp.setValue(default_ovp)
        self._set_ovp()
    ...
    # -------------------------------------------------------------------------
    # LOGGING SLOTS (stop timestamp filename)
    # -------------------------------------------------------------------------
    def _browse_log(self):
        path, _ = QFileDialog.getSaveFileName(self, "Set Log Base Name", self.log_path_edit.text(), "CSV (*.csv)")
        if path:
            # Remove timestamp if user picks a default? We'll keep as base.
            base, ext = os.path.splitext(path)
            # If the user chose a file ending with timestamp, keep as is, but we will append again.
            # Better to strip existing timestamp-like pattern if any.
    ...
    def _stop_logging(self):
        if self.log_thread and self.log_thread.isRunning():
            ...
            # Wait for thread to finish closing file
            self.log_thread.wait(5000)

            # Rename temporary file to final name with stop timestamp
            if self.temp_log_path and os.path.exists(self.temp_log_path):
                ...
                try:
                    ...
                    self.final_log_path = self.temp_log_path  # keep temp if rename fails
            else:
                self.final_log_path = None
    ...
    # -------------------------------------------------------------------------
    # PLOT APPEARANCE AND VISIBILITY
    # -------------------------------------------------------------------------
    def _update_plot_appearance(self):
        if not hasattr(self, 'ax'):
            return
        bg = self.combo_bg_color.currentText()
        if bg == "White":
            facecolor = "#ffffff"
            ax_color = "#ffffff"
            tick_color = "#000000"
        ...
        # Legend visibility
        self.ax.get_legend().set_visible(self.chk_legend.isChecked())

        # Labels and title
        self.ax.set_xlabel(self.edit_axis_x.text(), color=tick_color)
        self.ax.set_ylabel(self.edit_axis_y.text(), color=tick_color)
        self.ax.set_title(self.edit_title.text(), color=tick_color)
    ...
    # -------------------------------------------------------------------------
    # PLOT SAVING (current screen or full data)
    # -------------------------------------------------------------------------
    def _save_plot(self):
        # Ask user which save type
    ...
    def _save_full_data(self):
        ...
        # Ask which channels to include
        channel_dialog = QDialog(self)
        channel_dialog.setWindowTitle("Select Channels for Full Data Plot")
        layout = QVBoxLayout(channel_dialog)
        ...
        btn_box.accepted.connect(channel_dialog.accept)
        btn_box.rejected.connect(channel_dialog.reject)
        ...
        try:
            with open(self.final_log_path, 'r', newline='', encoding='utf-8') as f:
                ...
                # Create a new figure with current appearance settings
                ...
                ax_full.set_xlabel(self.edit_axis_x.text())
                ax_full.set_ylabel(self.edit_axis_y.text())
                ax_full.set_title(self.edit_title.text())
                if self.chk_legend.isChecked():
                    ax_full.legend(facecolor=self.fig.get_facecolor(), edgecolor='#3c3c3c',
                                   labelcolor='#d4d4d4' if self.combo_bg_color.currentText() == "Dark" else '#000000')
                ax_full.grid(self.chk_grid.isChecked())
    ...
    def _create_plot_figure(self):
        """Create a new figure with the same appearance settings as the live plot."""
        bg = self.combo_bg_color.currentText()
        if bg == "White":
            facecolor = "#ffffff"
            ax_color = "#ffffff"
