"""ScopeDisplayWidget — Multi-channel oscilloscope graticule display."""
import math
...
class ScopeDisplayWidget(QWidget):
    """
    ...
    def render_waveforms(self, wf_dict: Dict[str, Any]):
        """Render multichannel waveform dictionary from ScopeWorker."""
        time_arr = wf_dict.get("time", [])
        if not time_arr:
            return
        ...
        for ch in (1, 2, 3, 4):
            ...
            if key in wf_dict:
                v_data = wf_dict[key]
                if v_data and len(v_data) == len(time_arr):
                    self.set_channel_data(ch, time_arr, v_data)

        if "math" in wf_dict:
            m_data = wf_dict["math"]
            if m_data and len(m_data) == len(time_arr):
                self.set_channel_data(99, time_arr, m_data)

    def clear_sweeps(self):
        """Clear all plotted curves."""
        if HAVE_PYQTGRAPH:
            for curve in self.ch_curves.values():
                curve.setData([], [])
